...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    if (!self.options.disableRequestedAuthnContext) {
      ...
      if(self.options.authnContext instanceof Array) {
        for(var authnIndex=0; authnIndex<self.options.authnContext.length; authnIndex++) {
          AuthnContextClassRef.push({
            'saml:AuthnContextClassRef':{
              '@xmlns:saml': 'urn:oasis:names:tc:SAML:2.0:assertion',
              '#text': self.options.authnContext[authnIndex]
            }
          });
        }
      }
